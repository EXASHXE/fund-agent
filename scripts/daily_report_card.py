...
def parse():
    ...
    # Recs (new 9-col format)
    recs = []
    pr = re.compile(r"\|\s*\d+\s*\|\s*(\w{6})\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*(?:\|[^|]+?){4}\|\s*([^|]+?)\s*\|")
    ...
    if news_start >= 0:
        ...
        for blk in blocks:
            ...
            headlines = []
            hl_section = blk.split('**近期新闻精选：**')
            if len(hl_section) > 1:
                for line in hl_section[1].split('\n'):
                    m2 = re.match(r'- ([+\-○])\s*(?:\[([^\]]*)\])?\s*(.+)', line)
                    if m2:
                        hl_text = m2.group(3).strip()
                        if hl_text:
                            headlines.append(f"{m2.group(1)} {hl_text}")
            if headlines:
...
def MOOD_FROM_VAL(v):
    fv = float(v)
    if fv > 0.55: return "偏正面"
    if fv < 0.45: return "偏谨慎"
    return "中性"
...
def build_cards(d):
    ...
    els3 = [
        D(f"**市场情绪总览**\\n总新闻情绪均值 **{d['sent']}** | {MOOD_FROM_VAL(d['sent'])}"),
        H(),
        D("**逐基金情绪**\n" + "\n".join(sent_lines)),
    ]
