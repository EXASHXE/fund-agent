...
当未使用 `--use-agents` 标志时，沿用传统三因子框架：

- `quant_baseline.macro_score`（0-20） / `meso_score`（0-30） / `micro_score`（0-50）
- `agent_adjustments` 各分项限于 `[-10, +10]`
- 输出 `final_scores.{macro,meso,micro,total}`，total 等于三项之和
...
当使用 `--use-agents` 标志时，激活 5 维度评分引擎，每个维度 0-100 分：
...
|--------|-------|-------------|-------|----------|--------|------|
| NORMAL（默认） | 30% | 25% | 15% | 20% | 10% | 均衡配置，量化因子权重最高 |
| HIGH_VOLATILITY | 20% | 15% | 30% | 15% | 20% | 强调事件冲击和择时，降低基本面依赖 |
| TRENDING | 25% | 20% | 10% | 25% | 20% | 跟随趋势，提高持仓和择时权重 |
| CRISIS | 15% | 10% | 40% | 10% | 25% | 避险优先，事件和择时主导决策 |
...
```json
{
  ...
  "weights_used": {"quant": 0.30, "fundamental": 0.25, "event": 0.15, "position": 0.20, "timing": 0.10},
...
- 标准模式和增强模式的 JSON 字段不共存：同一份输出只用一种模式。
