...
from src.infra.config.shared import (
    dca_effective_date,
...
def compute_holdings(store, config, codes, analyzer=None):
    ...
    from legacy.engine.events import generate_events
    from legacy.engine.calculator import compute_fund
    from legacy.analysis.holdings import portfolio_summary
    ...
    for code in codes:
        try:
            ...
            if not nav_map:
                from src.infra.persistence.database import get_nav_history
...
def _simulate_dca_for_report(dca_strategy: dict, nav_map: dict, today) -> list:
    from legacy.engine.events import _generate_dca_dates
