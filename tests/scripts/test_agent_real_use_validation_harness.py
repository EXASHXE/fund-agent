...
class TestReportMdConditional:
    def test_report_absent_ok_when_valuation_blocked(self, synthetic_run_dir: Path):
    ...
    def test_report_required_when_analyze_done(self, synthetic_run_dir: Path):
...
class TestUnlockLanguageNegation:
    def test_not_an_unlock_switch_is_not_flagged(self, synthetic_run_dir: Path):
    ...
    def test_not_unlock_chinese_not_flagged(self, synthetic_run_dir: Path):
    ...
    def test_affirmative_unlock_is_flagged(self, synthetic_run_dir: Path):
...
class TestCanonicalEntrypointDetection:
    def test_canonical_run_passes(self, synthetic_run_dir: Path):
        result = _score_run(
            prompt="帮我做分析报告",
            exit_code=0,
            stdout="基金代码未验证",
    ...
    def test_non_canonical_manifest_fails(self, synthetic_run_dir: Path):
        manifest = json.loads((synthetic_run_dir / "run_manifest.json").read_text())
