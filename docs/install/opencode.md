...
```
fund-agent v0.10.0 plugin loaded; primary skill: fund-analysis;
...
`v0.10.0` and matches the `VERSION` file, the `package.json` `version`
...
cd fund-agent
git checkout v0.10.0 # if you cloned without --branch
```
