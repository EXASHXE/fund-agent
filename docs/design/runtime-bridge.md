...
> Status: **design only**. Not implemented in v0.4.5. This document
...
> `src/skills_runtime/`.
>
> The v0.4.5 OpenCode install is **plugin metadata + doc reader only**
> and does not run a runtime bridge. See
> [`docs/install/opencode.md`](../install/opencode.md) for the current
> install.

## Why a runtime bridge is a future concern, not a v0.4.5 one

The OpenCode plugin in v0.4.5 registers three custom tools
...
The v0.4.5 milestone therefore ships the **metadata + doc reader**
...
(likely `v0.5.x-runtime-bridge`). It is not on the v0.4.5
